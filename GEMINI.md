...
This file provides guidance to Google Antigravity and Gemini-based agents when working with code in this repository.

---

# Antigravity Workspace Profile: Manager OS

## Agent Autonomy & Execution Policy
- **Workspace Trust:** This project directory is fully trusted. 
- **Tool Permission Level:** always-proceed
- **Write Scope:** Subagents operating inside parallel Git worktrees have unconditional write authorization for all files under `frontend/src/` and `src/manager_os/`.
- **Review Policy:** Automatically approve intermediate Implementation Plans and Task Lists. Do not halt execution for human review unless an operation attempts to run a destructive command (e.g., deleting untracked baseline databases).
...
### Database Layer (`src/manager_os/db.py`)
...
### Extract (`src/manager_os/extract/`)
...
### Dashboard (`src/manager_os/dashboard/app.py`)
...
### Config (`src/manager_os/config.py`)
...
### Schemas (`src/manager_os/schemas.py`)
